...
float driveStraight_left(float delta) {
  float u_l_OL = (v_star + beta_left)/theta_left;
  float u_l_CL = u_l_OL - (f_left/theta_left)*delta;
  return u_l_CL;
}

float driveStraight_right(float delta) {
  float u_r_OL = (v_star + beta_right)/theta_right;
  float u_r_CL = u_r_OL + (f_right/theta_right)*delta;
  return u_r_CL;
}
...
/*    PREPROGRAMMED PATH     */
/*---------------------------*/
int run_times[NUM_COMMANDS] = {7000, 1700, 2500, 1600}; // length of commands roughly in ms
...
float delta_reference(int i) {
  ...
  if (drive_mode == DRIVE_RIGHT) { // Return a NEGATIVE expression
    return -(((v_star/5)*i*CAR_WIDTH)/TURN_RADIUS);
  }
  ...
  if (loop_mode == MODE_LISTEN && re_pointer == SIZE) {
    ...
    if(envelope(out, result)) {
      ...
      if (best_dist < EUCLIDEAN_THRESHOLD) {
        drive_mode = drive_modes[best_index-1];
        start_drive_mode();
